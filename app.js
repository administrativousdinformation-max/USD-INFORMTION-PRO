...
function iniciarSesionCompleta() {
    ...
    (function() {
        ...
        const mnHistPrueba  = document.getElementById('menuItemHistorialPrueba');

        const ocultarTodos = () => {
            [mnAsistencia, mnPuntos, mnRanking, mnCalidad, mnNomina, mnHV, mnChatPrueba, mnHistPrueba].forEach(el => { if(el) el.style.display = 'none'; });
        };
        ...
        if (rol === 'admin') {
            ...
            if(mnHistPrueba) mnHistPrueba.style.display = '';
                ...
                ${[['prueba1','Prueba 1','#3498db',cal1],['prueba2','Prueba 2','#9b59b6',cal2]].map(([slot,label,col,cal]) => {
                    ...
                    return `<div style="padding:12px 16px;border-right:1px solid rgba(255,255,255,0.05);">
                        ...
                        <details style="margin-top:8px;">
                            <summary style="font-size:10px;color:rgba(255,255,255,0.3);cursor:pointer;">${msgs.length} mensajes</summary>
                            <div style="padding-top:6px;max-height:150px;overflow-y:auto;">
                                ${msgs.map(m => `<div style="font-size:10px;margin-bottom:4px;"><span style="color:${m.de==='formador'?'#ff2d55':'rgba(255,255,255,0.5)'};">${m.nombreDe||m.de}:</span> ${m.texto}${m.img?` <img src="${m.img}" style="height:40px;vertical-align:middle;border-radius:4px;margin-left:4px;">`:''}</div>`).join('')}
                            </div>
                        </details>
                    </div>`;
                }).join('')}
...
function cpIniciarVista() {
    ...
    _cpCargarHistorial().then(() => {
        if (userLogueado.rol === 'capacitador') {
            cpRenderFormador();
        } else if (userLogueado.rol === 'admin' || userLogueado.rol === 'coordinador') {
            cpRenderObservador();
        } else {
            cpRenderCandidato();
        }
